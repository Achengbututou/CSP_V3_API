...
namespace demo.bll
{
    ...
    /// 描 述： 配车单数据库执行类
    /// </summary>
    public class EwMatecardetailService : ServiceBase
    {
        #region 获取数据
        /// <summary>
        /// 获取查询表达式
        ...
        private Expression<Func<EwMatecardetailEntity, bool>> GetExpression(EwMatecardetailEntity queryParams) {
            ...
            {
                expression = expression.And(t => t.F_matecarbasicId == queryParams.F_matecarbasicId);
            }
        ...
        public async Task DeleteRelate(string key) {
            await this.BaseRepository().Delete<EwMatecardetailEntity>(t=>t.F_matecarbasicId == key);
        }
        ...
        public async Task SaveList(string key, IEnumerable<EwMatecardetailEntity> list) {
            var addList = new List<EwMatecardetailEntity>();
            var db = this.BaseRepository().BeginTrans();
            try{
                await db.Delete<EwMatecardetailEntity>(t => t.F_matecarbasicId == key);
