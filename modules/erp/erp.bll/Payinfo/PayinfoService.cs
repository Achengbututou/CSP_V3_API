...
namespace erp.bll
{
    ...
    /// 描 述： lr_erp_payinfo数据库执行类
    /// </summary>
    public class PayinfoService : ServiceBase
    {
        #region 获取数据
        /// <summary>
        ...
        {
            var expression = LinqExtensions.True<Erp_payinfoEntity>();
            ...
            if(!string.IsNullOrEmpty(queryParams.F_PWId))
            {
                expression = expression.And(t => t.F_PWId.Contains(queryParams.F_PWId));
            }
            ...
            return this.BaseRepository().FindList<Erp_payinfoEntity>(expression);
        }
        ...
        public Task<IEnumerable<Erp_payinfoEntity>> GetPageList(Pagination pagination, Erp_payinfoEntity queryParams)
        {
            ...
            return this.BaseRepository().FindList<Erp_payinfoEntity>(expression,pagination);
        }
        ...
        {
            return this.BaseRepository().FindEntityByKey<Erp_payinfoEntity>(f_Id);
        }
        /// <summary>
        ...
        {
            return this.BaseRepository().FindList<Erp_payinfo_detailEntity>(t => t.F_PId == f_PId);
        }
        ...
        {
            var lr_erp_payinfoEntity = await this.BaseRepository().FindEntityByKey<Erp_payinfoEntity>(f_Id);
            var db = this.BaseRepository().BeginTrans();
            try
            {
                await db.Delete<Erp_payinfoEntity>(f_Id);
                await db.Delete<Erp_payinfo_detailEntity>(t=>t.F_PId == lr_erp_payinfoEntity.F_Id);
                db.Commit();
            }
            catch (Exception)
            {
        ...
        {
            await this.BaseRepository().Delete<Erp_payinfo_detailEntity>(t=>t.F_PId == f_PId);
        }
        ...
        public async Task SaveEntity(String f_Id,Erp_payinfoEntity lr_erp_payinfoEntity,IEnumerable<Erp_payinfo_detailEntity> lr_erp_payinfodetailList)
        {
            var db = this.BaseRepository().BeginTrans();
            try
            {
                ...
                {
                    lr_erp_payinfoEntity.F_Id = f_Id;
                    await db.Update(lr_erp_payinfoEntity);
                }
                else // 新增
                {
                    if(string.IsNullOrEmpty(lr_erp_payinfoEntity.F_Id))
                    {
                        lr_erp_payinfoEntity.F_Id = Guid.NewGuid().ToString();
                    }
                    await db.Insert(lr_erp_payinfoEntity);
                }
                foreach(var item in lr_erp_payinfodetailList)
                {
                    item.F_PId = lr_erp_payinfoEntity.F_Id;
        ...
        public async Task SaveLr_erp_payinfodetail(String f_Id,Erp_payinfo_detailEntity lr_erp_payinfodetailEntity)
        {
            if(!string.IsNullOrEmpty(f_Id))
            { // 更新
                lr_erp_payinfodetailEntity.F_Id = f_Id;
                await this.BaseRepository().Update(lr_erp_payinfodetailEntity);
            }
            else // 新增
            {
                lr_erp_payinfodetailEntity.F_Id = Guid.NewGuid().ToString();
                await this.BaseRepository().Insert(lr_erp_payinfodetailEntity);
            }
        }
