...
namespace erp.bll
{
    ...
    /// 描 述： lr_erp_salesoffer数据库执行类
    /// </summary>
    public class SalesOfferService : ServiceBase
    {
        #region 获取数据
        /// <summary>
        ...
        {
            var expression = LinqExtensions.True<Erp_sales_offerEntity>();
            ...
            return this.BaseRepository().FindList<Erp_sales_offerEntity>(expression);
        }
        ...
        public Task<IEnumerable<Erp_sales_offerEntity>> GetPageList(Pagination pagination, Erp_sales_offerEntity queryParams)
        {
            ...
            return this.BaseRepository().FindList<Erp_sales_offerEntity>(expression,pagination);
        }
        ...
        {
            return this.BaseRepository().FindEntityByKey<Erp_sales_offerEntity>(f_Id);
        }
        /// <summary>
        ...
        {
            return this.BaseRepository().FindList<Erp_sales_offer_detailEntity>(t => t.F_SFID == f_SFID);
        }
        ...
        {
            var lr_erp_salesofferEntity = await this.BaseRepository().FindEntityByKey<Erp_sales_offerEntity>(f_Id);
            var db = this.BaseRepository().BeginTrans();
            try
            {
                await db.Delete<Erp_sales_offerEntity>(f_Id);
                await db.Delete<Erp_sales_offer_detailEntity>(t=>t.F_SFID == lr_erp_salesofferEntity.F_Id);
                db.Commit();
            }
            catch (Exception)
            {
        ...
        {
            await this.BaseRepository().Delete<Erp_sales_offer_detailEntity>(t=>t.F_SFID == f_SFID);
        }
        ...
        public async Task SaveEntity(String f_Id,Erp_sales_offerEntity lr_erp_salesofferEntity,IEnumerable<Erp_sales_offer_detailEntity> lr_erp_salesofferdetailList)
        {
            var db = this.BaseRepository().BeginTrans();
            try
            {
                ...
                {
                    lr_erp_salesofferEntity.F_Id = f_Id;
                    await db.Update(lr_erp_salesofferEntity);
                }
                else // 新增
                {
                    if(string.IsNullOrEmpty(lr_erp_salesofferEntity.F_Id))
                    {
                        lr_erp_salesofferEntity.F_Id = Guid.NewGuid().ToString();
                    }
                    await db.Insert(lr_erp_salesofferEntity);
                }
                foreach(var item in lr_erp_salesofferdetailList)
                {
                    item.F_SFID = lr_erp_salesofferEntity.F_Id;
        ...
        public async Task SaveLr_erp_salesofferdetail(String f_Id,Erp_sales_offer_detailEntity lr_erp_salesofferdetailEntity)
        {
            if(!string.IsNullOrEmpty(f_Id))
            { // 更新
                lr_erp_salesofferdetailEntity.F_Id = f_Id;
                await this.BaseRepository().Update(lr_erp_salesofferdetailEntity);
            }
            else // 新增
            {
                lr_erp_salesofferdetailEntity.F_Id = Guid.NewGuid().ToString();
                await this.BaseRepository().Insert(lr_erp_salesofferdetailEntity);
            }
        }
