...
namespace erp.bll
{
    ...
    public class SalesOrderService : ServiceBase
    {
        ...
        {
            var expression = LinqExtensions.True<Erp_sales_orderEntity>();
            ...
            if(!string.IsNullOrEmpty(queryParams.F_SFId))
            {
                expression = expression.And(t => t.F_SFId.Contains(queryParams.F_SFId));
            }
            ...
            return this.BaseRepository().FindList<Erp_sales_orderEntity>(expression);
        }
        ...
        public Task<IEnumerable<Erp_sales_orderEntity>> GetPageList(Pagination pagination, Erp_sales_orderEntity queryParams)
        {
            ...
            return this.BaseRepository().FindList<Erp_sales_orderEntity>(expression,pagination);
        }
        ...
        {
            return this.BaseRepository().FindEntityByKey<Erp_sales_orderEntity>(f_Id);
        }
        /// <summary>
        ...
        {
            return this.BaseRepository().FindList<Erp_sales_order_detailEntity>(t => t.F_SOID == f_SOID);
        }
        ...
        {
            var lr_erp_salesorderEntity = await this.BaseRepository().FindEntityByKey<Erp_sales_orderEntity>(f_Id);
            var db = this.BaseRepository().BeginTrans();
            try
            {
                await db.Delete<Erp_sales_orderEntity>(f_Id);
                await db.Delete<Erp_sales_order_detailEntity>(t=>t.F_SOID == lr_erp_salesorderEntity.F_Id);
                db.Commit();
            }
            catch (Exception)
            {
        ...
        {
            await this.BaseRepository().Delete<Erp_sales_order_detailEntity>(t=>t.F_SOID == f_SOID);
        }
        ...
        public async Task SaveEntity(String f_Id,Erp_sales_orderEntity lr_erp_salesorderEntity,IEnumerable<Erp_sales_order_detailEntity> lr_erp_salesorderdetailList)
        {
            var db = this.BaseRepository().BeginTrans();
            try
            {
                ...
                {
                    lr_erp_salesorderEntity.F_Id = f_Id;
                    await db.Update(lr_erp_salesorderEntity);
                }
                else // 新增
                {
                    if(string.IsNullOrEmpty(lr_erp_salesorderEntity.F_Id))
                    {
                        lr_erp_salesorderEntity.F_Id = Guid.NewGuid().ToString();
                    }
                    await db.Insert(lr_erp_salesorderEntity);
                }
                foreach(var item in lr_erp_salesorderdetailList)
                {
                    item.F_SOID = lr_erp_salesorderEntity.F_Id;
        ...
        public async Task SaveLr_erp_salesorderdetail(String f_Id,Erp_sales_order_detailEntity lr_erp_salesorderdetailEntity)
        {
            if(!string.IsNullOrEmpty(f_Id))
            { // 更新
                lr_erp_salesorderdetailEntity.F_Id = f_Id;
                await this.BaseRepository().Update(lr_erp_salesorderdetailEntity);
            }
            else // 新增
            {
                lr_erp_salesorderdetailEntity.F_Id = Guid.NewGuid().ToString();
                await this.BaseRepository().Insert(lr_erp_salesorderdetailEntity);
            }
        }
