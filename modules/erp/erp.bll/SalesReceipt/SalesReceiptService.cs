...
namespace erp.bll
{
    ...
    /// 描 述： lr_erp_salesreceipt数据库执行类
    /// </summary>
    public class SalesReceiptService : ServiceBase
    {
        #region 获取数据
        /// <summary>
        ...
        {
            var expression = LinqExtensions.True<Erp_sales_receiptEntity>();
            ...
            if(queryParams.F_DeliveryDate != null && queryParams.F_DeliveryDate_end != null)
            {
                ...
                expression = expression.And(t => t.F_DeliveryDate >= f_DeliveryDate && t.F_DeliveryDate <= f_DeliveryDate);
            }
            ...
            if(!string.IsNullOrEmpty(queryParams.F_SOId))
            {
                expression = expression.And(t => t.F_SOId.Contains(queryParams.F_SOId));
            }
            ...
            return this.BaseRepository().FindList<Erp_sales_receiptEntity>(expression);
        }
        ...
        public Task<IEnumerable<Erp_sales_receiptEntity>> GetPageList(Pagination pagination, Erp_sales_receiptEntity queryParams)
        {
            ...
            return this.BaseRepository().FindList<Erp_sales_receiptEntity>(expression,pagination);
        }
        ...
        {
            return this.BaseRepository().FindEntityByKey<Erp_sales_receiptEntity>(f_Id);
        }
        /// <summary>
        ...
        {
            return this.BaseRepository().FindList<Erp_sales_receipt_detailEntity>(t => t.F_SRId == f_SRId);
        }
        ...
        {
            var lr_erp_salesreceiptEntity = await this.BaseRepository().FindEntityByKey<Erp_sales_receiptEntity>(f_Id);
            var db = this.BaseRepository().BeginTrans();
            try
            {
                await db.Delete<Erp_sales_receiptEntity>(f_Id);
                await db.Delete<Erp_sales_receipt_detailEntity>(t=>t.F_SRId == lr_erp_salesreceiptEntity.F_Id);
                db.Commit();
            }
            catch (Exception)
            {
        ...
        {
            await this.BaseRepository().Delete<Erp_sales_receipt_detailEntity>(t=>t.F_SRId == f_SRId);
        }
        ...
        public async Task SaveEntity(String f_Id,Erp_sales_receiptEntity lr_erp_salesreceiptEntity,IEnumerable<Erp_sales_receipt_detailEntity> lr_erp_salesreceiptdetailList)
        {
            var db = this.BaseRepository().BeginTrans();
            try
            {
                ...
                {
                    lr_erp_salesreceiptEntity.F_Id = f_Id;
                    await db.Update(lr_erp_salesreceiptEntity);
                }
                else // 新增
                {
                    if(string.IsNullOrEmpty(lr_erp_salesreceiptEntity.F_Id))
                    {
                        lr_erp_salesreceiptEntity.F_Id = Guid.NewGuid().ToString();
                    }
                    await db.Insert(lr_erp_salesreceiptEntity);
                }
                foreach(var item in lr_erp_salesreceiptdetailList)
                {
                    item.F_SRId = lr_erp_salesreceiptEntity.F_Id;
        ...
        public async Task SaveLr_erp_salesreceiptdetail(String f_Id,Erp_sales_receipt_detailEntity lr_erp_salesreceiptdetailEntity)
        {
            if(!string.IsNullOrEmpty(f_Id))
            { // 更新
                lr_erp_salesreceiptdetailEntity.F_Id = f_Id;
                await this.BaseRepository().Update(lr_erp_salesreceiptdetailEntity);
            }
            else // 新增
            {
                lr_erp_salesreceiptdetailEntity.F_Id = Guid.NewGuid().ToString();
                await this.BaseRepository().Insert(lr_erp_salesreceiptdetailEntity);
            }
        }
