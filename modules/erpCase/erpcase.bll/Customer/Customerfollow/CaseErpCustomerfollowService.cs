...
namespace erpCase.bll
{
    ...
    /// 描 述： 客户跟进数据库执行类
    /// </summary>
    public class CaseErpCustomerfollowService : ServiceBase
    {
        #region 获取数据
        /// <summary>
        /// 获取查询表达式
        /// </summary>
        /// <returns></returns>
        private Expression<Func<CaseErpCustomerfollowEntity, bool>> GetExpression(CaseErpCustomerfollowEntity queryParams) {
            var exp = Expressionable.Create<CaseErpCustomerfollowEntity>();
            ...
            if(!string.IsNullOrEmpty(queryParams.F_VisitTimeQRange))
            {
                var f_VisitTime_list = queryParams.F_VisitTimeQRange.Split(" - ");
                DateTime f_VisitTime = Convert.ToDateTime(f_VisitTime_list[0]);
                DateTime f_VisitTime_end = Convert.ToDateTime(f_VisitTime_list[1]);
                exp = exp.And(t => t.F_VisitTime >= f_VisitTime &&t.F_VisitTime <= f_VisitTime_end);
            }
            if(!string.IsNullOrEmpty(queryParams.F_NextVisitTimeQRange))
            {
                var f_NextVisitTime_list = queryParams.F_NextVisitTimeQRange.Split(" - ");
                DateTime f_NextVisitTime = Convert.ToDateTime(f_NextVisitTime_list[0]);
                DateTime f_NextVisitTime_end = Convert.ToDateTime(f_NextVisitTime_list[1]);
                exp = exp.And(t => t.F_NextVisitTime >= f_NextVisitTime &&t.F_NextVisitTime <= f_NextVisitTime_end);
            }
            if(queryParams.F_Content != null)
            {
                exp = exp.And(t => t.F_Content == queryParams.F_Content);
            }
        ...
        public Task<IEnumerable<CaseErpCustomerfollowEntity>> GetList(CaseErpCustomerfollowEntity queryParams)
        {
            ...
            return this.BaseRepository().FindList<CaseErpCustomerfollowEntity>(expression);
        }
        ...
        {
            var expression = GetExpression(queryParams);
            return this.BaseRepository().FindList<CaseErpCustomerfollowEntity>(expression,pagination);
        }
        ...
        public Task<CaseErpCustomerfollowEntity> GetEntity(string keyValue) {
            return this.BaseRepository().FindEntityByKey<CaseErpCustomerfollowEntity>(keyValue);
        }
        ...
        {
            await this.BaseRepository().Delete<CaseErpCustomerfollowEntity>(keyValue);
        }
        ...
        public async Task Deletes(string keyValues)
        {
            ...
            await this.BaseRepository().Delete<CaseErpCustomerfollowEntity>(keyValuesArr);
        }
