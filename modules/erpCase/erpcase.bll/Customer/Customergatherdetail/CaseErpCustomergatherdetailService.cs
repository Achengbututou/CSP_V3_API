...
namespace erpCase.bll
{
    ...
    /// 描 述： 客户回款详情数据库执行类
    /// </summary>
    public class CaseErpCustomergatherdetailService : ServiceBase
    {
        #region 获取数据
        /// <summary>
        /// 获取查询表达式
        /// </summary>
        /// <returns></returns>
        private Expression<Func<CaseErpCustomergatherdetailEntity, bool>> GetExpression(CaseErpCustomergatherdetailEntity queryParams) {
            var exp = Expressionable.Create<CaseErpCustomergatherdetailEntity>();
            exp = exp.AndIF(!string.IsNullOrEmpty(queryParams.F_GatherId),t => t.F_GatherId.Contains(queryParams.F_GatherId));
            if(queryParams.F_AmountCollect != null)
            {
                exp = exp.And(t => t.F_AmountCollect == queryParams.F_AmountCollect);
            }
        ...
        public Task<IEnumerable<CaseErpCustomergatherdetailEntity>> GetList(CaseErpCustomergatherdetailEntity queryParams)
        {
            ...
            return this.BaseRepository().FindList<CaseErpCustomergatherdetailEntity>(expression);
        }
        ...
        {
            var expression = GetExpression(queryParams);
            return this.BaseRepository().FindList<CaseErpCustomergatherdetailEntity>(expression,pagination);
        }
        ...
        public Task<CaseErpCustomergatherdetailEntity> GetEntity(string keyValue) {
            return this.BaseRepository().FindEntityByKey<CaseErpCustomergatherdetailEntity>(keyValue);
        }
        ...
        {
            await this.BaseRepository().Delete<CaseErpCustomergatherdetailEntity>(keyValue);
        }
        ...
        public async Task Deletes(string keyValues)
        {
            ...
            await this.BaseRepository().Delete<CaseErpCustomergatherdetailEntity>(keyValuesArr);
        }
