...
namespace erpCase.bll
{
    ...
    /// 描 述： 生产记录数据库执行类
    /// </summary>
    public class CaseErpProductlogService : ServiceBase
    {
        #region 获取数据
        /// <summary>
        /// 获取查询表达式
        /// </summary>
        /// <returns></returns>
        private Expression<Func<CaseErpProductlogEntity, bool>> GetExpression(CaseErpProductlogEntity queryParams) {
            var exp = Expressionable.Create<CaseErpProductlogEntity>();
        ...
        public Task<IEnumerable<CaseErpProductlogEntity>> GetList(CaseErpProductlogEntity queryParams)
        {
            ...
            return this.BaseRepository().FindList<CaseErpProductlogEntity>(expression);
        }
        ...
        {
            var expression = GetExpression(queryParams);
            return this.BaseRepository().FindList<CaseErpProductlogEntity>(expression,pagination);
        }
        ...
        public Task<CaseErpProductlogEntity> GetEntity(string keyValue) {
            return this.BaseRepository().FindEntityByKey<CaseErpProductlogEntity>(keyValue);
        }
        ...
        {
            await this.BaseRepository().Delete<CaseErpProductlogEntity>(keyValue);
        }
        ...
        public async Task Deletes(string keyValues)
        {
            ...
            await this.BaseRepository().Delete<CaseErpProductlogEntity>(keyValuesArr);
        }
