...
namespace erpCase.bll
{
    ...
    /// 描 述： 供应商时间轴数据库执行类
    /// </summary>
    public class CaseErpSupplytimeService : ServiceBase
    {
        #region 获取数据
        /// <summary>
        /// 获取查询表达式
        /// </summary>
        /// <returns></returns>
        private Expression<Func<CaseErpSupplytimeEntity, bool>> GetExpression(CaseErpSupplytimeEntity queryParams) {
            var exp = Expressionable.Create<CaseErpSupplytimeEntity>();
            ...
            exp = exp.AndIF(!string.IsNullOrEmpty(queryParams.F_ContentUser),t => t.F_ContentUser.Contains(queryParams.F_ContentUser));
            exp = exp.AndIF(!string.IsNullOrEmpty(queryParams.F_ContentReason),t => t.F_ContentReason.Contains(queryParams.F_ContentReason));
            if(queryParams.F_ContentExplain != null)
            {
                exp = exp.And(t => t.F_ContentExplain == queryParams.F_ContentExplain);
            }
        ...
        public Task<IEnumerable<CaseErpSupplytimeEntity>> GetList(CaseErpSupplytimeEntity queryParams)
        {
            ...
            return this.BaseRepository().FindList<CaseErpSupplytimeEntity>(expression,"F_CreateDate DESC");
        }
        ...
        {
            var expression = GetExpression(queryParams);
            return this.BaseRepository().FindList<CaseErpSupplytimeEntity>(expression,pagination);
        }
        ...
        public Task<CaseErpSupplytimeEntity> GetEntity(string keyValue) {
            return this.BaseRepository().FindEntityByKey<CaseErpSupplytimeEntity>(keyValue);
        }
        ...
        {
            await this.BaseRepository().Delete<CaseErpSupplytimeEntity>(keyValue);
        }
        ...
        public async Task Deletes(string keyValues)
        {
            ...
            await this.BaseRepository().Delete<CaseErpSupplytimeEntity>(keyValuesArr);
        }
