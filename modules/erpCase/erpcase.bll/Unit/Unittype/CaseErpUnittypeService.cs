...
namespace erpCase.bll
{
    ...
    /// 描 述： 单位类型数据库执行类
    /// </summary>
    public class CaseErpUnittypeService : ServiceBase
    {
        #region 获取数据
        /// <summary>
        /// 获取查询表达式
        /// </summary>
        /// <returns></returns>
        private Expression<Func<CaseErpUnittypeEntity, bool>> GetExpression(CaseErpUnittypeEntity queryParams) {
            var exp = Expressionable.Create<CaseErpUnittypeEntity>();
        ...
        public Task<IEnumerable<CaseErpUnittypeEntity>> GetList(CaseErpUnittypeEntity queryParams)
        {
            ...
            return this.BaseRepository().FindList<CaseErpUnittypeEntity>(expression);
        }
        ...
        {
            var expression = GetExpression(queryParams);
            return this.BaseRepository().FindList<CaseErpUnittypeEntity>(expression,pagination);
        }
        ...
        public Task<CaseErpUnittypeEntity> GetEntity(string keyValue) {
            return this.BaseRepository().FindEntityByKey<CaseErpUnittypeEntity>(keyValue);
        }
        ...
        {
            await this.BaseRepository().Delete<CaseErpUnittypeEntity>(keyValue);
        }
        ...
        public async Task Deletes(string keyValues)
        {
            ...
            await this.BaseRepository().Delete<CaseErpUnittypeEntity>(keyValuesArr);
        }
