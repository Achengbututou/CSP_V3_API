...
namespace mes.bll {
    ...
    /// 描 述： 成品异常报告数据库执行类
    /// </summary>
    public class MesFinishedExReportService: ServiceBase {
        ...
        /// <returns></returns>
        private Expression<Func<MesFinishedExReportEntity, bool>>GetExpression(MesFinishedExReportEntity queryParams) {
            var exp = Expressionable.Create<MesFinishedExReportEntity>();
        ...
        public Task<IEnumerable<MesFinishedExReportEntity>>GetList(MesFinishedExReportEntity queryParams) {
            ...
            return this.BaseRepository().FindList<MesFinishedExReportEntity>(expression);
        }
        /// <summary>
        /// 获取成品异常报告
        /// </summary>
        /// <param name="queryParams">查询参数</param>
        /// <returns></returns>
        public Task<IEnumerable<MesFinishedExReportEntity>> GetListByIds(List<string> ids)
        {
            return this.BaseRepository().FindList<MesFinishedExReportEntity>(t=>ids.Contains(t.F_Id));
        }
        /// <summary>
        /// 获取成品异常报告的分页数据
        /// </summary>
        ...
        public Task<IEnumerable<MesFinishedExReportEntity>>GetPageList(Pagination pagination, MesFinishedExReportEntity queryParams) {
            return this.BaseRepository().FindListByQueryable<MesFinishedExReportEntity>(q => {
                var queryable = q.InnerJoin<MesFinishedReportEntity>((t, t1) => t.F_FinishedId== t1.F_Id);
                var expression = GetExpression(queryParams);
                queryable = queryable.Where(expression);
                return queryable.Select((t, t1) => new MesFinishedExReportEntity()
                {
                    F_Id = t.F_Id,
        ...
        public Task<MesFinishedExReportEntity>GetEntity(string keyValue) {
            return this.BaseRepository().FindEntityByKey<MesFinishedExReportEntity>(keyValue);
        }
        #endregion
        ...
        public async Task Delete(string keyValue) {
            await this.BaseRepository().Delete<MesFinishedExReportEntity>(keyValue);
        }
        /// <summary>
        /// 批量删除实体
        /// </summary>
        ...
        public async Task Deletes(string keyValues) {
            ...
            await this.BaseRepository().Delete<MesFinishedExReportEntity>(keyValuesArr);
        }
        /// <summary>
        /// 保存数据
        /// </summary>
