...
namespace mes.bll {
    ...
    /// 描 述： 项目管理数据库执行类
    /// </summary>
    public class MesProjectInfoService: ServiceBase {
        ...
        /// <returns></returns>
        private Expression<Func<MesProjectInfoEntity, bool>>GetExpression(MesProjectInfoEntity queryParams) {
            var exp = Expressionable.Create<MesProjectInfoEntity>();
            exp = exp.AndIF(!string.IsNullOrEmpty(queryParams.F_ProjectCode), t => t.F_ProjectCode.Contains(queryParams.F_ProjectCode));
            if (queryParams.F_NumberState != null) {
                exp = exp.And(t => t.F_NumberState == queryParams.F_NumberState);
            }
            exp = exp.AndIF(!string.IsNullOrEmpty(queryParams.F_ProjectName), t => t.F_ProjectName.Contains(queryParams.F_ProjectName));
            exp = exp.AndIF(!string.IsNullOrEmpty(queryParams.F_ProjectType), t => t.F_ProjectType.Contains(queryParams.F_ProjectType));
            exp = exp.AndIF(!string.IsNullOrEmpty(queryParams.F_ProjectSource), t => t.F_ProjectSource.Contains(queryParams.F_ProjectSource));
            exp = exp.AndIF(!string.IsNullOrEmpty(queryParams.F_ProjectState), t => t.F_ProjectState.Contains(queryParams.F_ProjectState));
            exp = exp.AndIF(!string.IsNullOrEmpty(queryParams.F_ProjectPhase), t => t.F_ProjectPhase.Contains(queryParams.F_ProjectPhase));
            if (queryParams.F_EstimatedCost != null) {
                exp = exp.And(t => t.F_EstimatedCost == queryParams.F_EstimatedCost);
            }
            if (queryParams.F_TotalProject != null) {
                exp = exp.And(t => t.F_TotalProject == queryParams.F_TotalProject);
            }
        ...
        public Task<IEnumerable<MesProjectInfoEntity>>GetList(MesProjectInfoEntity queryParams) {
            ...
            return this.BaseRepository().FindList<MesProjectInfoEntity>(expression);
        }
        /// <summary>
        /// 获取项目管理的分页数据
        /// </summary>
        ...
        public Task<IEnumerable<MesProjectInfoEntity>>GetPageList(Pagination pagination, MesProjectInfoEntity queryParams) {
            var expression = GetExpression(queryParams);
            return this.BaseRepository().FindList<MesProjectInfoEntity>(expression, pagination);
        }
        /// <summary>
        /// 获取实体数据
        /// </summary>
        ...
        /// <returns></returns>
        public async Task<MesProjectInfoEntity>GetEntity(string keyValue)
        {
            var dataList = await this.BaseRepository().ORM.Queryable<MesProjectInfoEntity>()
                .LeftJoin<BaseUserEntity>((t, t1) => t.F_Principal == t1.F_UserId)
                .Where(t => t.F_Id == keyValue)
                .Select((t, t1) => new MesProjectInfoEntity
                {
                    F_Id = t.F_Id,
                    F_ProjectCode = t.F_ProjectCode,
                    F_NumberState = t.F_NumberState,
                    F_ProjectName = t.F_ProjectName,
                    F_ProjectType = t.F_ProjectType,
                    F_ProjectSource = t.F_ProjectSource,
                    F_ProjectState = t.F_ProjectState,
                    F_ProjectPhase = t.F_ProjectPhase,
                    F_EstimatedCost = t.F_EstimatedCost,
                    F_TotalProject = t.F_TotalProject,
        ...
        public async Task Delete(string keyValue) {
            await this.BaseRepository().Delete<MesProjectInfoEntity>(keyValue);
        }
        /// <summary>
        /// 批量删除实体
        /// </summary>
        ...
        public async Task Deletes(string keyValues) {
            ...
            await this.BaseRepository().Delete<MesProjectInfoEntity>(keyValuesArr);
        }
        /// <summary>
        /// 保存数据
        /// </summary>
